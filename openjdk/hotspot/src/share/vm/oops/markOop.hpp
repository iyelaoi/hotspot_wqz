...
//对象头 Mark word描述类，每个object都包含markOop
class markOopDesc: public oopDesc {
 ...
 public:
  ...
  ObjectMonitor* monitor() const {	//返回 ObjectMonitor* 对象指针
    assert(has_monitor(), "check");
    // Use xor instead of &~ to provide one extra tag-bit check.
    return (ObjectMonitor*) (value() ^ monitor_value);//value（）最低两位00，与重量级锁10异或
  }
