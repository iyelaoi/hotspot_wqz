...
class ObjectMonitor {
 ...
 public:
  ...
  ObjectMonitor() {
    ...
    _waiters      = 0, //等待线程数
    _recursions   = 0; //重入次数
    _object       = NULL; //监视器锁寄生的对象，锁寄托存储于对象中
    _owner        = NULL; //指向ObjectMonitor对象的线程或基础锁
    _WaitSet      = NULL; //处于Wait状态的线程，会被加入到wait set中
    _WaitSetLock  = 0 ;
    _Responsible  = NULL ;
    _succ         = NULL ;
    _cxq          = NULL ;
    FreeNext      = NULL ;
    _EntryList    = NULL ; //处于block的线程，会被加入到Entry set
    _SpinFreq     = 0 ;
    _SpinClock    = 0 ;
    OwnerIsThread = 0 ; //_owner is (Thread *) vs SP/BasicLock
    _previous_owner_tid = 0; //监视器前一个拥有者 线程ID
  }
