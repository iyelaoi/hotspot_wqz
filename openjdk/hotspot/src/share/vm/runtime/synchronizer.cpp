...
void ObjectSynchronizer::slow_enter(Handle obj, BasicLock* lock, TRAPS) {
  ...
  if (mark->is_neutral()) { //是否为无锁状态？
    ...
    if (mark == (markOop) Atomic::cmpxchg_ptr(lock, obj()->mark_addr(), mark)) {//CAS（尝试将MarkWord更新为指向BasicLock的指针）成功，释放栈锁
...
ObjectMonitor * ATTR ObjectSynchronizer::inflate (Thread * Self, oop object) {
  ...
  for (;;) {//自旋
      ...
      if (mark->has_monitor()) { //判断当前是否为重量级锁
          ObjectMonitor * inf = mark->monitor() ; //指向ObjectMonitor的指针
