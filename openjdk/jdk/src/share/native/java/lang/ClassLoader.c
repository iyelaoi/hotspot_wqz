...
Java_java_lang_ClassLoader_00024NativeLibrary_load
  (JNIEnv *env, jobject this, jstring name, jboolean isBuiltin)//JNIEnv封装了本地方法
{
    const char *cname;
    jint jniVersion;
    jthrowable cause;
    ...
    cname = JNU_GetStringPlatformChars(env, name, 0);//名称格式转换
    if (cname == 0)
        return;
    handle = isBuiltin ? procHandle : JVM_LoadLibrary(cname); //1. 先加载本地类库文件，调用系统函数dlopen读取库文件：如os::dll_load()(跟操作系统相关)
    if (handle) {
        JNI_OnLoad_t JNI_OnLoad;
        JNI_OnLoad = (JNI_OnLoad_t)findJniFunction(env, handle,
                                               isBuiltin ? cname : NULL,
                                               JNI_TRUE);//去库中查找JNI_OnLoad方法地址
        if (JNI_OnLoad) {
            JavaVM *jvm;
            (*env)->GetJavaVM(env, &jvm);//获取虚拟机对象
            jniVersion = (*JNI_OnLoad)(jvm, NULL);//JNI_Onload方法返回JNI的version版本
        } else {
